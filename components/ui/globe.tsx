...
// Kéo dài module để sử dụng ThreeGlobe trong @react-three/fiber
...
// Định nghĩa cấu hình cho bản đồ toàn cầu
...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  // Tạo các điểm và vật liệu cho bản đồ
  useEffect(() => {
    if (globeRef.current) {
      _buildData();
      _buildMaterial();
  ...
  const startAnimation = () => {
    ...
    globeRef.current
      .ringsData([]) // Đặt dữ liệu cho vòng
  ...
  // Cập nhật vòng và hiệu ứng sau mỗi 2 giây
  useEffect(() => {
    if (!globeRef.current || !globeData) return;
